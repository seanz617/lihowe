...
if [ $2 == 'bootstrap' ];then
	...
	if [ ! -n "$4" ] ;then
		./cmd/$2/$2 daemon --rpchost=$ip --bindip=$ip --config=$cfg_file --daemon &> $work_dir/regnet-logs/$1/$2$3/logs
	else
		./cmd/$2/$2 daemon --rpchost=$ip --bindip=$ip --config=$cfg_file --daemon --testnet=$4 &> $work_dir/regnet-logs/$1/$2$3/logs
	fi		
fi
...
if [ $2 == 'center' ]; then
	...
	echo "starting $2$3 process"
	if [ ! -n "$4" ] ;then
		./$2 --rpchost=$ip --rpcport=$rpc_port --tcpport=$tcp_port --udpport=$udp_port --redishost=$redis_host --redisport=$redis_port --redisdb=$redis_db --bindip=$ip --daemon &> $work_dir/regnet-logs/$1/$2$3/logs
	else
		./$2 --rpchost=$ip --rpcport=$rpc_port --tcpport=$tcp_port --udpport=$udp_port --redishost=$redis_host --redisport=$redis_port --bindip=$ip --daemon --test=$4 &> $work_dir/regnet-logs/$1/$2$3/logs
	fi
fi
...
if [ $2 == 'verifier' ]; then
	...
	if [ ! -n "$4" ] ;then
		./$2 --daemon --rpchost=$ip --bindip=$ip --config=$cfg_file &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	else
		./$2 --daemon --rpchost=$ip --bindip=$ip --config=$cfg_file --test=$4 &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	fi
fi
...
if [ $2 == 'indexer' ]; then
	...
	if [ ! -n "$4" ] ;then
		./indexer --bindip=$ip --rpchost=$ip --daemon --config=$cfg_file &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	else
		./indexer --bindip=$ip --rpchost=$ip --daemon --config=$cfg_file --test=$4 &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	fi
fi
...
if [ $2 == 'gateway' ]; then
	...
	echo "./$2 --rpchost=$ip --bindip=$ip --rpcport=$rpc_port --tcpport=$tcp_port --udpport=$udp_port --indexerip=$indexer_ip --indexertcpport=$indexer_tcp_port --indexerudpport=$indexer_udp_port --indexerid=$indexer_id --nasnodeaddress=$gppio_host:$gppio_port --daemon &> $work_dir/regnet-logs/$1/$2$3/logs"
	if [ ! -n "$4" ] ;then
		./$2 --rpchost=$ip --bindip=$ip --rpcport=$rpc_port --tcpport=$tcp_port --udpport=$udp_port --indexerip=$indexer_ip --indexertcpport=$indexer_tcp_port --indexerudpport=$indexer_udp_port --indexerid=$indexer_id --nasnodeaddress=$gppio_host:$gppio_port --daemon &> $work_dir/regnet-logs/$1/$2$3/logs
	else
		./$2 --rpchost=$ip --bindip=$ip --rpcport=$rpc_port --tcpport=$tcp_port --udpport=$udp_port --paymentip=$payment_ip --paymenttcpport=$payment_tcp_port --paymentudpport=$payment_udp_port --nasnodeaddress=$gppio_host:$gppio_port --daemon --testnet=$4 &> $work_dir/regnet-logs/$1/$2$3/logs
	fi
fi
...
if [ $2 == 'user' ]; then
	...
	if [ ! -n "$4" ] ;then
		./poss start-daemon --daemon --datadir=$config_dir/$1/$2$3 --config=$config_dir/$1/$2$3/poss.conf &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	else
		./poss start-daemon --daemon --datadir=$config_dir/$1/$2$3 --config=$config_dir/$1/$2$3/poss.conf --testnet=$4 &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	fi
fi
...
if [ $2 == 'miner' ]; then
	...
	cd $work_dir/$go_enfi_dir
	if [ ! -n "$4" ] ;then
		#./$2 --daemon --datadir=$config_dir/$1/$2$3 --config=$config_dir/$1/$2$3/miner.conf &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
		./$2 --daemon --datadir=$config_dir/$1/$2$3 --config=$config_dir/$1/$2$3/miner.conf --storage-chiprice=100 --download-chiprice=100 &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	else
		./$2 --daemon --datadir=$config_dir/$1/$2$3 --config=$config_dir/$1/$2$3/miner.conf --testnet=$4 &> $work_dir/regnet-logs/$1/$2$3/logs </dev/null
	fi
fi
